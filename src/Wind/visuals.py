...
from PyQt6.QtGui import QPainter, QPainterPath, QColor, QPen, QFont, QFontMetrics, QRegion

# Importaciones de tus módulos
from .logic import aniquilar_bordes_dwm, obtener_limites_pantalla, calcular_nueva_posicion
...
class Mascota(QWidget):
    def __init__(self):
        ...
        QTimer.singleShot(1000, lambda: self.decir(f"Modelo: {self.modelo_actual} v{self.version}"))
    ...
    def cargar_nueva_accion(self, accion_forzada=None, decir=True):
        info = self.cerebro.decidir_proxima_accion(en_chat=self.ventana_chat.isVisible(), accion_forzada=accion_forzada)
    ...
    def pulso_vital(self):
        ...
        if self.arrastrando:
            ...
            if self.estado_actual in ["WALK", "RUN"]:
                ...
                if self.pos_x <= 0: self.mirando_derecha = True
                elif self.pos_x >= self.limite_derecho: self.mirando_derecha = False
    ...
    def mouseMoveEvent(self, event):
        if self.arrastrando:
            nx, ny = calcular_nueva_posicion(event.globalPosition().toPoint(), self.offset_mouse)
            self.pos_x = nx
            self.pos_y = ny
            self.posicionar_burbuja()
            if self.ventana_chat.isVisible():
    ...
    def mostrar_menu(self, pos):
        menu = QMenu(self)
        menu.setStyleSheet("background-color: #00BCD4; color: white; font-weight: bold;")
        ...
        if seleccion == act_chat:
            ...
            self.cerebro.guardar_todo() # <-- FIX: Ahora llama a guardar_todo()
            QApplication.quit()
